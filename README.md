...
`wtm` makes `git worktree` pleasant to use day to day. It is a native
desktop app for macOS and Linux, with a CLI and full-screen TUI alongside it
for the terminal. All three discover worktrees straight from git's own
registry (so it works no matter where they live on disk, even ones created
by a raw `git worktree add`), compute status (dirty/ahead/behind/merged) in
parallel, and stay out of your way.

<p align="center">
  <img src="assets/app.png" alt="wtm's desktop app: a sidebar of repositories, a worktree list with status pills and dirty file counts, and a detail panel" width="840">
</p>
...
The native app is the easiest way to use wtm: every repository in a
sidebar, every worktree with its status, and create/switch/prune without a
terminal. It is ad-hoc signed, not notarized, so the first launch on macOS
is right-click → **Open** (details in [App](#app)).

- **macOS (Apple Silicon):** download
  [`WTM-macOS.zip`](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-macOS.zip),
  unzip, and drag `WTM.app` to `/Applications`.
- **Linux x86_64:**
  [`.deb`](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-linux-x86_64.deb)
  (`sudo apt install ./WTM-linux-x86_64.deb`) or the
  [tarball](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-linux-x86_64.tar.xz)
  with its bundled `install.sh`.
- **Linux ARM64:**
  [`.deb`](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-linux-aarch64.deb)
  or the
  [tarball](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-linux-aarch64.tar.xz).

Every link above always points at the newest release; the
[releases page](https://github.com/codenameakshay/wtm-manager/releases)
has older versions and checksums.

### Command-line tool via installer script

```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/codenameakshay/wtm-manager/releases/latest/download/wtm-installer.sh | sh
```

The script downloads the right prebuilt `wtm` binary for your platform and
puts it on your `PATH`. Binaries are published for:
...
[cargo-dist](https://opensource.axo.dev/cargo-dist/). With the app
installed, bare `wtm` on a terminal opens it.
...
- Download
  [`WTM-macOS.zip`](https://github.com/codenameakshay/wtm-manager/releases/latest/download/WTM-macOS.zip)
  (Apple Silicon) and unzip it.
- Or build it yourself: `scripts/bundle-mac.sh` assembles `WTM.app` from
  source (see [`scripts/README.md`](scripts/README.md)). Install with
