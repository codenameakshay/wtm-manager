...
- **`head_time` on `wtm list --json`.** Unix seconds of the HEAD commit,
  filled from the main repository so `--fast` still includes it.
...
- **The desktop app is the headline.** The website and README now lead
  with the app: a direct download for macOS (`WTM-macOS.zip`) and Linux
  (`.deb` and tarball, x86_64 and ARM64) sits first, and the CLI's
  one-line installer follows as the terminal companion. The release
  assets themselves are unchanged.
